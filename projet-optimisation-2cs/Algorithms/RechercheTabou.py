...
def Rech_Tabou(cap_sac, array, nb_it, nb_itchng, lt):
    items_ord = []
    items = []
    s = len(array)
    ...
    while((It <= It_max) and (It_nochange < It_max_nochange)):
        ...
        if (Capacity(curr_sol, items_ord, s, cap_sac) < min_poids(items_ord, cap_sac, s)):
            ...
            while(j < int(s)):
                ...
                sol1 = Voisin(curr_sol, k, i, j, cap_sac, items_ord, s)
                so = sol1
        ...
        if not(sol_voi in T):  # Est ce que cette solution n'est pas dans la liste tabou
            ...
            # et améliore la meilleure solution actuelle
            if (Objfun(best_sol, items_ord, s) < Objfun(sol_voi, items_ord, s)):
        ...
        if(count_tabu == tl):  # si longueur de la liste tabou est atteinte
            ...
            count_tabu -= 1
        T.append(curr_sol)
    M = Objfun(best_sol, items_ord, s)
    tps2 = time.time()
    tps = tps2-tps1
    Benefice = M
    capacite_prise = cap_sac-Capacity(best_sol, items_ord, s, cap_sac)
    resultat = sortie_sol(best_sol, items_ord)
